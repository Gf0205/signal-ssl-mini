...
def pretrain_denoise_backbone_for_seed(
...
def print_summary(rows: list[dict]) -> None:
    ...
    denoise_by_seed = {row["seed"]: row for row in rows if row["method"] == "denoise_pretrained"}

    print("\n=== Per-seed delta: Denoise - Scratch ===")
    for seed in sorted(scratch_by_seed):
        acc_delta = denoise_by_seed[seed]["test_acc"] - scratch_by_seed[seed]["test_acc"]
        f1_delta = denoise_by_seed[seed]["test_macro_f1"] - scratch_by_seed[seed]["test_macro_f1"]
...
def main() -> None:
    ...
    parser.add_argument("--checkpoint-dir", type=Path, default=Path("checkpoints/denoise_multiseed"))
    parser.add_argument("--out", type=Path, default=Path("results/denoise_multiseed_results.csv"))
    ...
    print("=== Multi-seed denoising reliability run ===")
    ...
    for seed in args.seeds:
        ...
        ckpt_path, ssl_train_loss, ssl_val_loss = pretrain_denoise_backbone_for_seed(
        ...
        print(
            f"Denoising pretrain done | train_clean_mse={ssl_train_loss:.6f} | "
            f"val_clean_mse={ssl_val_loss:.6f}"
        )
    ...
    print("\nPASS: multi-seed denoising reliability check completed.")
